...
A proportionate next step is an external HTTP monitor every five minutes for the homepage and `/start-a-project`, checking status and expected page content. Pair it with a periodic browser check that opens the form, validates required fields, and verifies a translated route. Actual Netlify receipt and notification delivery need a separate synthetic submission and a test recipient/filtering policy. The HTTP validator checks form markup only and never submits inquiries.

Choose the monitor and alert recipient before enabling recurring checks. No production monitor or alert destination is configured by this change. Enable failure notifications in [GitHub notification settings](https://github.com/settings/notifications) for CI; GitHub documents how [workflow notifications](https://docs.github.com/en/actions/concepts/workflows-and-actions/notifications-for-workflow-runs) depend on the triggering user and notification preferences. [Netlify deploy notifications](https://docs.netlify.com/deploy/deploy-notifications/) cover deployment events, not general uptime. Scheduled GitHub workflows can be delayed and are not a replacement for a dedicated uptime monitor.
...
| P1 | CI is prepared, but continuous live-site monitoring and an alert recipient still need configuration. Use the small monitoring plan above. | Small scope for uptime; medium for browser/delivery checks. Separate operational setup. |
